...
class FileChooserCore {
	...
	/**
	 * A file that indicates the a_file_dialog_folder that is currently being displayed.
	 */
	...
	/**
	 * A boolean indicating if the a_file_dialog_folder's full path must be show in the title.
	 */
	...
	/**
	 * Static attribute for save the a_file_dialog_folder displayed by default.
	 */
	...
	/**
	 * Implementation of the click listener for when the a_file_dialog_add button is clicked.
	 */
	...
	private FileItem.OnFileClickListener fileItemClickListener = new FileItem.OnFileClickListener() {
		public void onClick(FileItem source) {
			// Verify if the item is a a_file_dialog_folder.
			File file = source.getFile();
			if(file.isDirectory()) {
				// Open the a_file_dialog_folder.
	 ...
	 * 
	 * @param listener The listener to a_file_dialog_add.
	 */
	...
	public interface OnFileSelectedListener {
		 ...
		 * 
		 * @param folder The file's parent a_file_dialog_folder.
	 ...
	 * 
	 * @param file The file or a_file_dialog_folder selected or the a_file_dialog_folder in which the file must be created.
	...
	private void notifyListeners(final File file, final String name) {
		...
		if((creation && this.showConfirmationOnCreate || !creation && this.showConfirmationOnSelect)) {
			...
			// Set the message and the 'yes' and 'a_file_dialog_no' buttons.
			alert.setMessage( message );
			alert.setPositiveButton(posButton, new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int whichButton) {
					// Notify to listeners.
	...
	/**
	 * Allows to define if, in the title, must be show only the current a_file_dialog_folder's name or the full file's path..
	 * 
	...
	public void setLabels(FileChooserLabels labels) {
		...
		// Verify if the buttons for a_file_dialog_add a file or select a a_file_dialog_folder has been modified.
		if(labels != null) {
			LinearLayout root = this.chooser.getRootLayout();
	...
	/**
	 * Returns the current a_file_dialog_folder.
	 * 
	 * @return The current a_file_dialog_folder.
	 */
	...
	/**
	 * Loads all the files of a a_file_dialog_folder in the file chooser.
	 * 
	 * If a_file_dialog_no path is specified ('folderPath' is null) the root a_file_dialog_folder of the SD card is going to be used.
	 * 
	 * @param folderPath The a_file_dialog_folder's path.
	 */
	 ...
	 * Loads all the files of a a_file_dialog_folder in the file chooser.
	 * 
	 * If a_file_dialog_no path is specified ('a_file_dialog_folder' is null) the root a_file_dialog_folder of the SD card is going to be used.
	 * 
	 * @param folder The a_file_dialog_folder.
	 */
	public void loadFolder(File folder) {
		...
		if(this.currentFolder.exists() && layout != null) {
			...
			// Add the parent a_file_dialog_folder.
			...
			if(this.currentFolder.isDirectory()) {
				// Get the a_file_dialog_folder's files.
				...
				if(fileList != null) {
					...
					// Iterate all the files in the a_file_dialog_folder.
					for(int i=0; i<fileList.length; i++) {
						// Verify if file can be selected (is a directory or a_file_dialog_folder mode is not activated and the file pass the filter, if defined).
						...
						if(selectable || !this.showOnlySelectable) {
							// Create the file item and a_file_dialog_add it to the list.
				...
				// Set the name of the current a_file_dialog_folder.
				...
				// The file is not a a_file_dialog_folder, a_file_dialog_add only this file.
				fileItems.add(new FileItem(this.chooser.getContext(), this.currentFolder));
			}
	
			
			// Add click listener and a_file_dialog_add the FileItem objects to the layout.
			for(int i=0; i<fileItems.size(); i++) {
				fileItems.get(i).addListener(this.fileItemClickListener);
				layout.addView(fileItems.get(i));
			}
			
			// Refresh default a_file_dialog_folder.
